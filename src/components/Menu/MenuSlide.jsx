...
export default function MenuSlide() {
    ...
    return (
        <div className="mt-[-6rem] w-full box-border overflow-hidden container mx-auto py-20 flex flex-row items-center">
            <div className="w-[300px] h-[10rem] shrink-0 px-4 text-gray-600 border-l-4 border-blue-500 bg-white py-2 z-10 sm:text-base text-xl font-semibold text-left">Sweets</div>

            <div className="flex animate-marquee whitespace-nowrap">
                {foods.map((food, index) => (
                    <img key={index} src={food}  className="mx-12 w-[10rem] h-[10rem] rounded-[50%] object-cover" />
                ))}

                {foods.map((food, index) => (
                    <img key={`duplicate-${index}`} src={food} className="mx-12 w-[10rem] rounded-[50%] h-[10rem] object-cover" /> 
                ))}
            </div>
        </div>
    )
